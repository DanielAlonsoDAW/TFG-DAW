(function (u, m) {
  typeof exports == "object" && typeof module < "u"
    ? m(exports, require("leaflet"))
    : typeof define == "function" && define.amd
    ? define(["exports", "leaflet"], m)
    : ((u = typeof globalThis < "u" ? globalThis : u || self),
      m((u["leaflet-control-geocoder"] = {}), u.L));
})(this, function (u, m) {
  "use strict";
  var ut = Object.defineProperty;
  var ht = (u, m, v) =>
    m in u
      ? ut(u, m, { enumerable: !0, configurable: !0, writable: !0, value: v })
      : (u[m] = v);
  var p = (u, m, v) => ht(u, typeof m != "symbol" ? m + "" : m, v);
  function v(a) {
    ...
    return (t.default = a), Object.freeze(t);
  }
  const s = v(m);
  function h(a, t) {
  ...
  function J(a, t) {
    return a.replace(/\{ *([\w_]+) *\}/g, (n, e) => {
      let o = t[e];
      return (
        o === void 0 ? (o = "") : typeof o == "function" && (o = o(t)), H(o)
      );
    });
  }
  class x {
    ...
    async geocode(t) {
      const n = h(this.options, {
        ...
        e = await d(this.options.serviceUrl + "/findAddressCandidates", n),
        o = [];
      if (e.candidates && e.candidates.length)
        for (let i = 0; i <= e.candidates.length - 1; i++) {
          const r = e.candidates[i],
            l = s.latLng(r.location.y, r.location.x),
            c = s.latLngBounds(
              s.latLng(r.extent.ymax, r.extent.xmax),
              s.latLng(r.extent.ymin, r.extent.xmin)
            );
          o[i] = { name: r.address, bbox: c, center: l };
        }
      return o;
    }
    suggest(t) {
    ...
    async reverse(t, n) {
      const e = g(this.options, {
        ...
        o = await d(this.options.serviceUrl + "/reverseGeocode", e),
        i = [];
      if (o && !o.error) {
        const r = s.latLng(o.location.y, o.location.x),
          l = s.latLngBounds(r, r);
        i.push({ name: o.address.Match_addr, center: r, bbox: l });
      }
      return i;
    }
  }
  ...
  class U {
    ...
    async reverse(t, n) {
      const e = g(this.options, { key: this.options.apiKey }),
        o = await d(this.options.serviceUrl + t.lat + "," + t.lng, e),
        i = [];
      for (let r = o.resourceSets[0].resources.length - 1; r >= 0; r--) {
        const l = o.resourceSets[0].resources[r],
          c = l.bbox;
        i[r] = {
          name: l.name,
  ...
  class C {
    ...
    async geocode(t) {
      const n = {
        ...
        e = this.options.serviceUrl + "/address/json",
        o = await d(e, n),
        i = [];
  ...
  class R {
    ...
    async geocode(t) {
      const n = h(this.options, { key: this.options.apiKey, address: t }),
        e = await d(this.options.serviceUrl, n),
        o = [];
      if (e.results && e.results.length)
        for (let i = 0; i <= e.results.length - 1; i++) {
          const r = e.results[i],
            l = s.latLng(r.geometry.location),
            c = s.latLngBounds(
    ...
    async reverse(t, n) {
      ...
      if (o.results && o.results.length)
        for (let r = 0; r <= o.results.length - 1; r++) {
          const l = o.results[r],
            c = s.latLng(l.geometry.location),
            f = s.latLngBounds(
  ...
  class E {
    constructor(t) {
      ...
      if ((s.Util.setOptions(this, t), t != null && t.apiKey))
    ...
    reverse(t, n) {
      ...
      this.options.reverseGeocodeProxRadius &&
        (e += "," + this.options.reverseGeocodeProxRadius);
    ...
    async getJSON(t, n) {
      ...
      if (e.response.view && e.response.view.length)
        for (let i = 0; i <= e.response.view[0].result.length - 1; i++) {
          const r = e.response.view[0].result[i].location,
            l = s.latLng(
    ...
    async getJSON(t, n) {
      ...
      if (e.items && e.items.length)
        for (let i = 0; i <= e.items.length - 1; i++) {
          const r = e.items[i],
            l = s.latLng(r.position.lat, r.position.lng);
          let c;
          r.mapView
            ? (c = s.latLngBounds(
  ...
  function X(a) {
    return a != null && a.apiKey ? new k(a) : new E(a);
  }
  function D(a) {
    let t;
    if (
  ...
  class B {
    constructor(t) {
      p(this, "options", { next: void 0, sizeInMeters: 1e4 });
      s.Util.setOptions(this, t);
    }
    async geocode(t) {
      const n = D(t);
      return n
        ? [{ name: t, center: n, bbox: n.toBounds(this.options.sizeInMeters) }]
        : this.options.next
        ? this.options.next.geocode(t)
        : [];
    }
  }
  function Y(a) {
  ...
  class O {
    ...
    _getProperties(t) {
      const n = { text: t.text, address: t.address };
      for (let e = 0; e < (t.context || []).length; e++) {
        const o = t.context[e].id.split(".")[0];
        (n[o] = t.context[e].text),
          t.context[e].short_code &&
            (n.countryShortCode = t.context[e].short_code);
      }
      return n;
    }
    async geocode(t) {
      const n = this.options.serviceUrl + encodeURIComponent(t) + ".json",
        e = h(this.options, { access_token: this.options.apiKey });
      e.proximity !== void 0 &&
        e.proximity.lat !== void 0 &&
        e.proximity.lng !== void 0 &&
        (e.proximity = e.proximity.lng + "," + e.proximity.lat);
      const o = await d(n, e);
    ...
    async reverse(t, n) {
      const e = this.options.serviceUrl + t.lng + "," + t.lat + ".json",
        o = g(this.options, { access_token: this.options.apiKey }),
        i = await d(e, o);
      return this._parseResults(i);
    }
    _parseResults(t) {
      var e;
      if (!((e = t.features) != null && e.length)) return [];
      const n = [];
      for (let o = 0; o <= t.features.length - 1; o++) {
        const i = t.features[o],
          r = s.latLng(i.center.reverse());
        let l;
        i.bbox
          ? (l = s.latLngBounds(
    ...
    _parseResults(t) {
      ...
      if (t.results && t.results[0].locations)
        for (let e = t.results[0].locations.length - 1; e >= 0; e--) {
          const o = t.results[0].locations[e],
            i = s.latLng(o.latLng);
          n[e] = {
  ...
  class K {
    ...
    async geocode(t) {
      const n = h(this.options, {
        ...
        e = await d(this.options.serviceUrl + "geocode-address", n),
        o = [];
      if (e.locations) {
        e.geometry = e.locations[0];
        const i = s.latLng(e.geometry.latitude, e.geometry.longitude),
          r = s.latLngBounds(i, i);
        o[0] = { name: e.geometry.address, bbox: r, center: i };
      }
      return o;
    }
    suggest(t) {
    ...
    async reverse(t, n) {
      const e = g(this.options, {
        ...
        o = await d(this.options.serviceUrl + "geocode-reverse", e),
        i = [];
      if (o.status.status == 200 && o.found) {
        const r = s.latLng(t.lat, t.lng),
          l = s.latLngBounds(r, r);
        i[0] = { name: o.address, bbox: l, center: r };
      }
      return i;
    }
  }
      ...
      p(this, "options", {
        ...
        htmlTemplate(t) {
          ...
          return (
            ...
            (n.city || n.town || n.village || n.hamlet) &&
              ((e =
                o.length > 0 ? "leaflet-control-geocoder-address-detail" : ""),
              o.push(
                '<span class="' +
                  e +
            ...
            (n.state || n.country) &&
              ((e =
                o.length > 0 ? "leaflet-control-geocoder-address-context" : ""),
              o.push('<span class="' + e + '">{state} {country}</span>')),
            J(o.join("<br/>"), n)
          );
        },
      });
  ...
  class T {
    ...
    async geocode(t) {
      const n = h(this.options, { key: this.options.apiKey, q: t }),
        e = await d(this.options.serviceUrl, n);
      return this._parseResults(e);
    }
    suggest(t) {
      ...
      if (t.results && t.results.length)
        for (let e = 0; e < t.results.length; e++) {
          ...
          o.annotations && o.annotations.bounds
            ...
            : (r = s.latLngBounds(i, i)),
            n.push({ name: o.formatted, bbox: r, center: i });
        }
      return n;
    }
  }
  ...
  class _ {
    constructor(t) {
      p(this, "options", { serviceUrl: "https://api.geocode.earth/v1" });
      s.Util.setOptions(this, t);
    }
    async geocode(t) {
      const n = h(this.options, { api_key: this.options.apiKey, text: t }),
        e = await d(this.options.serviceUrl + "/search", n);
        ...
        s.geoJSON(t, {
          ...
          onEachFeature(o, i) {
            ...
            i.getBounds
              ...
              : ((c = i.getLatLng()), (l = s.latLngBounds(c, c))),
              (r.name = i.feature.properties.label),
              (r.center = c),
              (r[n] = l),
              (r.properties = i.feature.properties),
              e.push(r);
          },
        }),
        e
  ...
  class M {
    ...
    async reverse(t, n) {
      const e = g(this.options, { lat: t.lat, lon: t.lng }),
    ...
    _parseResults(t) {
      ...
      if (t && t.features)
        for (let o = 0; o < t.features.length; o++) {
          const i = t.features[o],
            r = i.geometry.coordinates,
            l = s.latLng(r[1], r[0]),
            c = (e = i.properties) == null ? void 0 : e.extent,
            f = c
              ? s.latLngBounds([c[1], c[0]], [c[3], c[2]])
              : s.latLngBounds(l, l);
          n.push({
            name: this._decodeFeatureName(i),
            html: this.options.htmlTemplate
              ? this.options.htmlTemplate(i)
              : void 0,
            center: l,
            bbox: f,
  ...
  class N {
    constructor(t) {
      p(this, "options", { serviceUrl: "https://api.what3words.com/v2/" });
      s.Util.setOptions(this, t);
    }
    async geocode(t) {
      const n = await d(
        ...
        e = [];
      if (n.geometry) {
        const o = s.latLng(n.geometry.lat, n.geometry.lng),
          i = s.latLngBounds(o, o);
        e[0] = { name: n.words, bbox: i, center: o };
      }
      return e;
    }
    suggest(t) {
    ...
    async reverse(t, n) {
      ...
      if (e.status.status == 200) {
        const i = s.latLng(e.geometry.lat, e.geometry.lng),
          r = s.latLngBounds(i, i);
        o[0] = { name: e.words, bbox: r, center: i };
      }
      return o;
    }
  }
  ...
  s.Util.extend(w.prototype, s.Control.prototype),
    s.Util.extend(w.prototype, s.Evented.prototype);
  class b extends w {
    constructor(n) {
      super(n);
      p(this, "options", {
      ...
      s.Util.setOptions(this, n),
        this.options.geocoder || (this.options.geocoder = new y());
    }
    addThrobberClass() {
    ...
    onAdd(n) {
      var c;
      const e = "leaflet-control-geocoder",
        o = s.DomUtil.create("div", e + " leaflet-bar"),
        i = s.DomUtil.create("button", e + "-icon", o),
        r = s.DomUtil.create("div", e + "-form", o);
      (this._map = n),
        (this._container = o),
        (i.innerHTML = "&nbsp;"),
        (i.type = "button"),
        i.setAttribute("aria-label", this.options.iconLabel);
      const l = (this._input = s.DomUtil.create("input", "", r));
      return (
        (l.type = "search"),
        (l.value = this.options.query),
        (l.placeholder = this.options.placeholder),
        s.DomEvent.disableClickPropagation(l),
        (this._errorElement = s.DomUtil.create("div", e + "-form-no-error", o)),
        ...
        s.DomEvent.addListener(l, "keydown", this._keydown, this),
        (c = this.options.geocoder) != null &&
          c.suggest &&
          s.DomEvent.addListener(l, "input", this._change, this),
        s.DomEvent.addListener(l, "blur", () => {
        ...
        this.options.collapsed
          ? this.options.expand === "click"
            ? s.DomEvent.addListener(o, "click", (f) => {
                f.button === 0 && f.detail !== 2 && this._toggle();
              })
            ...
            ? s.DomEvent.addListener(
                ...
                (f) => {
                  this._toggle(), f.preventDefault(), f.stopPropagation();
                },
                this
              )
            : (s.DomEvent.addListener(o, "mouseover", this._expand, this),
              s.DomEvent.addListener(o, "mouseout", this._collapse, this),
              this._map.on("movestart", this._collapse, this))
          : (this._expand(),
            s.Browser.touch
              ? s.DomEvent.addListener(o, "touchstart", () => this._geocode())
              : s.DomEvent.addListener(o, "click", () => this._geocode())),
        ...
        s.DomEvent.disableClickPropagation(o),
        o
      );
    }
    setQuery(n) {
    ...
    async _geocode(n = !1) {
      ...
      if (!n && e.length < this.options.queryMinLength) return;
      const o = ++this._requestCount;
      (this._lastGeocode = e), n || this._clearResults();
      const i = { input: e };
      this.fire(n ? "startsuggest" : "startgeocode", i);
      const r = n
        ? await this.options.geocoder.suggest(e)
        : await this.options.geocoder.geocode(e);
      if (o === this._requestCount) {
        const l = { input: e, results: r };
        this.fire(n ? "finishsuggest" : "finishgeocode", l),
          this._geocodeResult(r, n);
      }
    }
    ...
    _toggle() {
      s.DomUtil.hasClass(this._container, "leaflet-control-geocoder-expanded")
        ? this._collapse()
        : this._expand();
    }
    _expand() {
      s.DomUtil.addClass(this._container, "leaflet-control-geocoder-expanded"),
        this._input.select(),
        this.fire("expand");
    }
    _collapse() {
    ...
    _createAlt(n, e) {
      const o = s.DomUtil.create("li", ""),
        i = s.DomUtil.create("a", "", o),
        r =
          this.options.showResultIcons && n.icon
            ? s.DomUtil.create("img", "", i)
            : null,
        l = n.html ? void 0 : document.createTextNode(n.name),
        c = (f) => {
          (this._preventBlurCollapse = !0),
            s.DomEvent.stop(f),
            this._geocodeResultSelected(n),
            s.DomEvent.on(o, "click touchend", () => {
              this.options.collapsed ? this._collapse() : this._clearResults();
            });
        };
      return (
        r && (r.src = n.icon),
        o.setAttribute("data-result-index", String(e)),
        n.html ? (i.innerHTML = i.innerHTML + n.html) : l && i.appendChild(l),
        s.DomEvent.addListener(o, "mousedown touchstart", c, this),
        o
      );
    }
    _keydown(n) {
      const e = (o) => {
        this._selection &&
          ...
          (this._selection =
            this._selection[o > 0 ? "nextSibling" : "previousSibling"])),
          this._selection ||
            (this._selection = this._alts[o > 0 ? "firstChild" : "lastChild"]),
          this._selection &&
            s.DomUtil.addClass(
              this._selection,
              "leaflet-control-geocoder-selected"
    ...
    _change() {
      ...
      n !== this._lastGeocode &&
        ...
        n.length >= this.options.suggestMinLength
   ...
   * All rights reserved.
   */ s.Util.extend(b, A),
    s.Util.extend(s.Control, { Geocoder: b, geocoder: G }),
    (u.Geocoder = b),
    (u.default = b),
    (u.geocoder = G),
    (u.geocoders = A),
